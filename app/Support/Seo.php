    ...
    public static function shouldIndex(array $viewData): bool
    {
        try {
            ...
            if (isset($viewData['author']) && isset($viewData['articles'])) {
                if (is_object($viewData['articles']) && method_exists($viewData['articles'], 'isEmpty') && $viewData['articles']->isEmpty()) {
                    return false;
                }
            }
    ...
    public static function generateSchema(array $viewData): string
    {
        try {
            ...
            if ($article) {
                ...
                $isRecipe = false;
                if ($article->category && isset($article->category->slug) && (Str::contains(strtolower($article->category->slug), ['recipe', 'food', 'cooking', 'kitchen']) || Str::contains(strtolower($article->title ?? ''), ['recipe', 'how to cook', 'how to make']))) {
                    $isRecipe = true;
                }

                // Check if it has FAQ structure
                $faqs = self::parseFaqs($article->body ?? '');
                $publishedIso = optional($article->published_at)->toIso8601String() ?? optional($article->created_at)->toIso8601String() ?? now()->toIso8601String();
                $updatedIso = optional($article->updated_at)->toIso8601String() ?? $publishedIso;
                ...
                $finalFaqs = array_merge($faqs, is_array($article->faq_items) ? $article->faq_items : []);
                if (!empty($finalFaqs)) {
                    $faqSchema = [
                        '@context' => 'https://schema.org',
                        '@type' => 'FAQPage',
