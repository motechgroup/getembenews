    ...
    <aside x-data="{ sidebarOpen: false }" class="w-full md:w-64 bg-gray-900 text-gray-300 flex flex-col border-r border-gray-800 shrink-0">
        ...
        <nav :class="{ 'hidden': !sidebarOpen, 'block': sidebarOpen }" class="hidden md:block flex-grow p-4 space-y-4 overflow-y-auto max-h-[calc(100vh-8rem)] scrollbar-thin scrollbar-thumb-gray-800">
            ...
            <div class="space-y-1">
                ...
                <a href="/admin/announcements" class="flex items-center space-x-3 px-3 py-2 text-xs font-semibold rounded hover:bg-gray-800 hover:text-white transition {{ request()->is('admin/announcements*') ? 'bg-gray-800 text-white border-l-4 border-[#C8102E] pl-2' : '' }} w-full">
                    <svg class="h-4 w-4 shrink-0 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
