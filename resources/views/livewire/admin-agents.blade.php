<div class="space-y-6 text-xs">
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-gray-50 dark:bg-gray-850 text-[10px] text-gray-500 dark:text-gray-400 font-bold uppercase tracking-wider border-b border-gray-100 dark:border-gray-800">
                        <th class="py-3.5 px-4">Agent / Business</th>
                ...
                <tbody class="divide-y divide-gray-150 dark:divide-gray-800 font-semibold text-gray-770 dark:text-gray-300">
                    @forelse($agents as $agent)
                        <tr class="hover:bg-orange-50/40 dark:hover:bg-gray-800/60 transition cursor-pointer group" wire:click="viewDetails({{ $agent->id }})">
                            <td class="py-4 px-4">
                                <div class="font-bold text-gray-900 dark:text-white group-hover:text-[#cc6c3b] transition flex items-center gap-2">
                                    <span>{{ $agent->name }}</span>
                                    <span class="text-[9px] bg-gray-100 dark:bg-gray-800 text-gray-500 px-1.5 py-0.5 rounded font-mono font-normal">ID: #{{ $agent->id }}</span>
                                </div>
                                @if($agent->business_name)
                                    <div class="text-[10px] text-[#cc6c3b] font-semibold flex items-center gap-1 mt-0.5">
                            ...
                            <td class="py-4 px-4 font-mono">
                                @if($agent->phone)
                                    <div class="text-xs text-gray-800 dark:text-gray-200 font-bold flex items-center gap-1.5">
                                        <svg class="w-3.5 h-3.5 text-[#cc6c3b]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"></path></svg>
                                        {{ $agent->phone }}
                                    </div>
                                @else
                                    <span class="text-[10px] text-red-400 italic">No Phone Number</span>
                                @endif
                            </td>
                            <td class="py-4 px-4">
                                <span class="text-xs text-gray-700 dark:text-gray-300 font-medium">{{ $agent->location }}</span>
                            </td>
                            <td class="py-4 px-4">
                                <span class="bg-orange-50 dark:bg-orange-950/30 text-[#cc6c3b] px-2 py-1 rounded text-xs font-bold border border-orange-200 dark:border-orange-900/40">
                                    {{ $agent->commission_percentage }}%
                                </span>
                            </td>
                            <td class="py-4 px-4 text-right space-x-2">
                                <button type="button" 
                                        wire:click.stop="viewDetails({{ $agent->id }})" 
                                        class="bg-[#cc6c3b]/10 hover:bg-[#cc6c3b] text-[#cc6c3b] hover:text-white font-bold text-xs px-3 py-1.5 rounded-lg transition inline-flex items-center gap-1">
                                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg>
                                    View Details
                                </button>
                                <button type="button" 
                                        wire:click.stop="openForm({{ $agent->id }})" 
                                        class="text-blue-600 hover:text-blue-800 font-bold px-1.5 py-1">Edit</button>
                                <button type="button" 
                                        ...
                                        wire:click.stop="deleteAgent({{ $agent->id }})" 
                                        class="text-red-600 hover:text-red-800 font-bold px-1.5 py-1">Delete</button>
                            </td>
                        </tr>
                    @empty
                        <tr>
    ...
    @if($isDetailsOpen && $selectedAgentForDetails)
        <div class="fixed inset-0 z-50 overflow-y-auto bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 sm:p-6" wire:click.self="closeDetails()">
            <div class="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl p-6 shadow-2xl space-y-6 w-full max-w-5xl max-h-[90vh] overflow-y-auto" @click.stop>
                <div class="flex justify-between items-start border-b border-gray-100 dark:border-gray-800 pb-4">
                    <div>
                        <div class="flex items-center gap-2">
                            <h2 class="text-base font-serif font-black uppercase text-gray-900 dark:text-white tracking-wider">
                                Agent Profile: {{ $selectedAgentForDetails->name }}
                            </h2>
                            <span class="bg-[#cc6c3b]/10 text-[#cc6c3b] text-[10px] font-bold px-2 py-0.5 rounded font-mono">ID: #{{ $selectedAgentForDetails->id }}</span>
                        </div>
                        <p class="text-xs text-gray-550 dark:text-gray-400 mt-1 flex flex-wrap items-center gap-3 font-medium">
                            @if($selectedAgentForDetails->business_name)
                                <span>Business: <strong class="text-gray-800 dark:text-gray-200">{{ $selectedAgentForDetails->business_name }}</strong></span> &bull;
                            @endif
                            @if($selectedAgentForDetails->phone)
                                <span>Phone: <strong class="font-mono text-gray-800 dark:text-gray-200">{{ $selectedAgentForDetails->phone }}</strong></span> &bull;
                            @else
                                <span>Phone: <em class="text-red-400 font-mono">Not Provided</em></span> &bull;
                            @endif
                            <span>Location: <strong>{{ $selectedAgentForDetails->location }}</strong></span> &bull;
                            <span>Commission Rate: <strong class="text-[#cc6c3b]">{{ $selectedAgentForDetails->commission_percentage }}%</strong></span> &bull;
                            <span>Security PIN: <strong class="font-mono text-[#cc6c3b]">{{ $selectedAgentForDetails->pin }}</strong></span>
                            <button type="button" 
                                    ...
                                    class="text-[10px] font-bold text-[#cc6c3b] hover:underline bg-orange-50 dark:bg-orange-950/30 px-2 py-0.5 rounded border border-orange-200 dark:border-orange-900/40">
                                ↻ Reset PIN
                            </button>
                        </p>
                    </div>
                    <button type="button" wire:click="closeDetails()" class="bg-gray-100 hover:bg-gray-200 dark:bg-gray-800 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-300 font-bold px-3 py-1.5 rounded-lg transition text-xs uppercase tracking-wider flex items-center gap-1">
