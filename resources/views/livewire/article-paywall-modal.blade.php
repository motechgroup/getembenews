<div class="my-8 rounded-2xl bg-gradient-to-b from-gray-900 via-gray-950 to-black text-white p-6 sm:p-8 border border-gray-800 shadow-2xl relative overflow-hidden" 
    ...
    <div class="absolute top-0 right-0 -mt-10 -mr-10 w-48 h-48 bg-red-600/10 rounded-full blur-3xl pointer-events-none"></div>
    <div class="absolute bottom-0 left-0 -mb-10 -ml-10 w-48 h-48 bg-emerald-600/10 rounded-full blur-3xl pointer-events-none"></div>

    <!-- Header / Paywall Title -->
    <div class="relative z-10 text-center space-y-3 max-w-xl mx-auto">
        <div class="inline-flex items-center space-x-2 px-3 py-1 bg-red-600/20 border border-red-500/30 rounded-full text-[#C8102E] text-[11px] font-black uppercase tracking-widest">
        ...
        <h3 class="text-2xl sm:text-3xl font-serif font-black text-white leading-tight">
    ...
    <div class="relative z-10 grid grid-cols-2 sm:grid-cols-4 gap-3 mt-8 max-w-2xl mx-auto">
        ...
        <button type="button" wire:click="selectOption('article')" 
                class="p-4 rounded-xl border text-left transition relative flex flex-col justify-between {{ $selectedOption === 'article' ? 'border-[#C8102E] bg-red-950/30 ring-2 ring-[#C8102E]/50' : 'border-gray-800 bg-gray-900/60 hover:bg-gray-850' }}">
            <div>
                <span class="text-[10px] font-extrabold uppercase tracking-wider text-gray-400 block">This Story</span>
                <span class="text-lg font-black text-white mt-1 block">KSh {{ number_format($this->getPriceForOption('article')) }}</span>
            </div>
            <span class="text-[9px] text-gray-400 mt-2 block">One-time payment</span>
        </button>

        <!-- Option 2: Daily Pass -->
        <button type="button" wire:click="selectOption('daily')" 
                class="p-4 rounded-xl border text-left transition relative flex flex-col justify-between {{ $selectedOption === 'daily' ? 'border-[#C8102E] bg-red-950/30 ring-2 ring-[#C8102E]/50' : 'border-gray-800 bg-gray-900/60 hover:bg-gray-850' }}">
            <div>
                <span class="text-[10px] font-extrabold uppercase tracking-wider text-emerald-400 block">Daily Pass</span>
                <span class="text-lg font-black text-white mt-1 block">KSh {{ number_format($this->getPriceForOption('daily')) }}</span>
            </div>
            <span class="text-[9px] text-gray-400 mt-2 block">24 hours access</span>
        </button>

        <!-- Option 3: Weekly Pass -->
        <button type="button" wire:click="selectOption('weekly')" 
                class="p-4 rounded-xl border text-left transition relative flex flex-col justify-between {{ $selectedOption === 'weekly' ? 'border-[#C8102E] bg-red-950/30 ring-2 ring-[#C8102E]/50' : 'border-gray-800 bg-gray-900/60 hover:bg-gray-850' }}">
            ...
            <div>
                <span class="text-[10px] font-extrabold uppercase tracking-wider text-emerald-400 block">Weekly Pass</span>
                <span class="text-lg font-black text-white mt-1 block">KSh {{ number_format($this->getPriceForOption('weekly')) }}</span>
            </div>
            <span class="text-[9px] text-gray-400 mt-2 block">7 days access</span>
        </button>

        <!-- Option 4: Monthly Pass -->
        <button type="button" wire:click="selectOption('monthly')" 
                class="p-4 rounded-xl border text-left transition relative flex flex-col justify-between {{ $selectedOption === 'monthly' ? 'border-[#C8102E] bg-red-950/30 ring-2 ring-[#C8102E]/50' : 'border-gray-800 bg-gray-900/60 hover:bg-gray-850' }}">
            <div>
                <span class="text-[10px] font-extrabold uppercase tracking-wider text-yellow-400 block">Monthly Pass</span>
                <span class="text-lg font-black text-white mt-1 block">KSh {{ number_format($this->getPriceForOption('monthly')) }}</span>
            </div>
            <span class="text-[9px] text-gray-400 mt-2 block">30 days access</span>
        </button>
    </div>
