...
<div x-data="{
     ...
     <div @click.away="dismiss()" class="relative w-full max-w-md bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-2xl shadow-2xl overflow-hidden p-6 sm:p-8 space-y-4">
         <!-- Close Button -->
         <button @click="dismiss()" class="absolute top-4 right-4 text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 transition">
         ...
         <div class="flex items-center space-x-3">
             <div class="p-3 bg-red-50 dark:bg-red-950/20 text-[#C8102E] rounded-full">
         ...
         <!-- Error Alert -->
         @if($errorMessage)
             <div class="p-2.5 bg-red-900/10 border border-red-800 text-red-300 text-[11px] rounded">
                 {{ $errorMessage }}
             </div>
         @endif
         ...
         @if($subscribed)
             <div class="space-y-2 py-4 text-center" x-init="setTimeout(() => { dismiss(); }, 2000)">
                 ...
                 <p class="text-[11px] text-gray-500">Thank you for joining Getembe News mailing list.</p>
             </div>
         @else
             <!-- Form View -->
             <form wire:submit.prevent="subscribe" class="space-y-3">
                 ...
                 <button type="submit" class="w-full py-2.5 bg-[#C8102E] hover:bg-red-700 text-white font-bold text-xs rounded-lg transition shadow-md shadow-red-900/20">
                     Subscribe Now
                 </button>
                 
                 <p class="text-[10px] text-gray-400 text-center">We respect your privacy. Unsubscribe at any time.</p>
             </form>
         @endif
     </div>
</div>
